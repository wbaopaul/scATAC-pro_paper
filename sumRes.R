...
if(dtype == 'HSC_GSE96769') true.label = readRDS('data/intermediate/HSC_GSE96769/cell_type.rds')
...
dtype = 'bonemarrow_clean'  ## or erythropoiesis 
...
methods = c('seurat', 'seurat_correct',  'cisTopic', 'scABC', 'chromVAR', 'LSI', 'SCRAT')
res = sumMethods4default(true.label, dtype, methods)
res = updateSummary(true.label, dtype, 'chromVAR', res$rands, res$pred.labels)
res = updateSummary(true.label, dtype, 'cisTopic', res$rands, res$pred.labels)
rands = res$rands
pred.labels = res$pred.labels
...
tt = fread('data/intermediate/time4resample10k_hsc.txt')
methods = c('seurat',  'cisTopic', 'scABC', 'chromVAR', 'LSI', 'SCRAT')
setkey(tt, V1)
tt = tt[methods, ]

postscript(file = paste0('output/figures/time4resample10k_hsc.eps'),
           width = 10, height = 8)
barplot(tt$V2, col = set.cols, ylab = 'Computational Time (s)', names.arg = tt$V1 )
dev.off()

## run with different composition ####
dtype = 'resample10k_hsc'
rands = list()
methods = c('seurat', 'seurat_correct', 'cisTopic', 'scABC', 'chromVAR', 'LSI', 'SCRAT')

for(method0 in methods){
  fname = paste0('data/intermediate/randIndexs/rand4', method0, '_resample10k_hsc.txt')
